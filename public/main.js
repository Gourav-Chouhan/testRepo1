...
const putMessage = (data, side, sendTo) => {
  ...
  let dataToDatabase = {};
  if (!data) {
    sentBy.textContent = username;
    dataToDatabase.username = username;
    dataToDatabase.data = _input.value;
  } else {
  ...
  time.textContent = timeString + " " + currTime.toLocaleDateString();
  if (data) {
    time.textContent = data.sentTime ? data.sentTime : time.textContent;
  }
  dataToDatabase.sentTime = time.textContent;

  if (!data) {
    socket.emit("toDatabase", dataToDatabase);
  }
...
async function getGif() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    img.addEventListener("click", () => {
      putMessage(
        { username: username, data: `<img src="${img.dataset.gifUrl}" \>` },
        "right"
      );
